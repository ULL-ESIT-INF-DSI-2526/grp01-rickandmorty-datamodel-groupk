import { beforeEach, describe, expect, test } from "vitest";

import { Low } from "lowdb";
import { Data } from "../../src/DataBase/db";
import { JSONFilePreset } from "lowdb/node";
import { defaultData } from "../../src/DataBase/db";

import { InventServices } from "../../src/Servicies/InventServices"; 
import { Invents } from "../../src/Class/Invents";
import { Character } from "../../src/Class/Character";
import { Species } from "../../src/Class/Species";
import { Dimensions } from "../../src/Class/Dimensions";
import { DimensionState } from "../../src/Enums/DimensionState";

describe("Test para la clase LocationServices", () => {
  let dimensionRick: Dimensions;
  let dimensionMorty: Dimensions;
  let humanSpecie: Species;
  let rick: Character;
  let morty: Character;
  let invent1: Invents;
  let invent2: Invents
  let invents: InventServices;

  beforeEach(async () => {
    let newdb: Low<Data> = await JSONFilePreset("src/DataBase/dbTest.json", defaultData); //Creamos nueva base de datos para los test, así no se sobreescribe la original
    
    dimensionRick = new Dimensions("D-001", "C-137", DimensionState.ACTIVA, 10, "Dimensión original");
    dimensionMorty = new Dimensions("D-002", "Dimensión Cronenberg", DimensionState.CUARENTENA, 5, "Arruinada");
    humanSpecie = new Species("S-001", "Humano", dimensionRick, "Mamífero", 80, "Bípedo estándar");
    rick = new Character("C-001", "Rick Sanchez", humanSpecie, dimensionRick, "Vivo", "Ninguna", 10, "Científico genio");
    morty = new Character("C-002", "Morty Smith", humanSpecie, dimensionMorty, "Vivo", "Ninguna", 4, "Nieto influenciable");
    invent1 = new Invents("I-001", "Portal Gun", rick, "Transporte", 10, "Viaje interdimensional");
    invent2 = new Invents("I-002", "Plumbus", morty, "Hogar", 1, "Todos tienen uno");

    newdb.data.invents = [invent1];
    await newdb.write();

    invents = new InventServices(newdb);
  });
  test("Debe obtener todos, añadir y eliminar inventos correctamente", async () => {
    await invents.add(invent2);
    expect((await invents.getAll()).length).toBe(2);
    await expect(invents.add(invent1)).rejects.toThrow("No se puede añadir ese invento porque ya existe");
    await invents.remove("I-001");
    expect((await invents.getAll()).length).toBe(1);
    await expect(invents.remove("No existe")).rejects.toThrow("No se puede eliminar un invento que no existe");
  });

  test("Debe modificar las propiedades de los inventos correctamente", async () => {
    await invents.add(invent2);
    // Modificamos múltiples campos a la vez
    await invents.modify("I-001", { 
      name: "Pistola Rota", 
      type: "Arma", 
      inventor: morty, 
      dangerLevel: 9, 
      desc: "Va a explotar" 
    });

    // Modificamos campo a campo 
    await invents.modify("I-002", { name: "Mega Plumbus" });
    await invents.modify("I-002", { type: "Herramienta" });
    await invents.modify("I-002", { inventor: rick });
    await invents.modify("I-002", { dangerLevel: 5 });
    await invents.modify("I-002", { desc: "Más útil ahora" });

    const inventsList: Invents[] = await invents.getAll();

    // Comprobamos el primer invento
    expect(inventsList[0].id).toBe("I-001"); 
    expect(inventsList[0].name).toBe("Pistola Rota");
    expect(inventsList[0].type).toBe("Arma");
    expect(inventsList[0].inventor).toBe(morty);
    expect(inventsList[0].dangerLevel).toBe(9);
    expect(inventsList[0].desc).toBe("Va a explotar");

    // Comprobamos el segundo invento
    expect(inventsList[1].name).toBe("Mega Plumbus");
    expect(inventsList[1].type).toBe("Herramienta");
    expect(inventsList[1].inventor).toBe(rick);
    expect(inventsList[1].dangerLevel).toBe(5);
    expect(inventsList[1].desc).toBe("Más útil ahora");

    // Comprobamos los returns (true/false) del método modify
    const modificacionExitosa = await invents.modify("I-001", { name: "Test" });
    expect(modificacionExitosa).toBe(true);
    const modificacionFallida = await invents.modify("ID INVENTADO", { name: "Fallo" });
    expect(modificacionFallida).toBe(false);
  });

  test("Debe realizar consultas de inventos correctamente", async () => {
    await invents.add(invent2);
    expect(await invents.consultInventByName("Plumbus")).toStrictEqual([invent2]);
    expect(await invents.consultInventByType("Transporte")).toStrictEqual([invent1]);
    expect(await invents.consultInventByInventor(rick)).toStrictEqual([invent1]);
    expect(await invents.consultInventByDangerLevel(1)).toStrictEqual([invent2]);
  });
});